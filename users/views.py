...
def facebook_insights_view(request):
    ...
    if not page_info:
        return JsonResponse(
            {'error': 'Failed to retrieve page access token or page ID'}, 
            status=400
        )
    ...
    if metrics_param:
        ...
        if not valid:
            return JsonResponse(
                {'error': f'Invalid metrics requested: {", ".join(invalid_metrics)}'}, 
                status=400
            )
    ...
    params = {'metric': ','.join(metrics), 'access_token': page_access_token}
